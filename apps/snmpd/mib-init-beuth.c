...
s8t getSysDescr(mib_object_t* object, u8t* oid, u8t len)
{
    if (!object->varbind.value.p_value.len) {
        object->varbind.value.p_value.ptr = (u8t*)"Contiki 2.6 with SNMPv3";
        object->varbind.value.p_value.len = 23;
    }
    return 0;
}
...
s8t mib_init()
{
    ...
    if (add_scalar(&oid_system_desc, 0, BER_TYPE_OCTET_STRING, 0, &getSysDescr, &setSysDescr) == -1 ||
        ...
        add_scalar(&oid_system_sysName, 0, BER_TYPE_OCTET_STRING, "@Raven SNMPv3", 0, 0) == -1 ||
